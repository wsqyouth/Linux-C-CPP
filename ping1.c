...
int ping_status(char *ipStr);    //ipStr可为IP，如ping失败则返回-1
...
int main(int argc, char * argv[]){
    ...
    //argv[1]是IP或者域名字符串
	int res = ping_status(argv[1]);
    if(res < 0)
    {
		printf("---disconnect %s server\n",argv[1]);
    }else
    {
		printf("---connected %s server\n",argv[1]);
	}
...
int ping_status(char *ipStr)  
{
    ...
    if(inaddr = inet_addr(ipStr) == INADDR_NONE){
    ...
    printf("ping %s (%s) : %d bytes of data.\n", ipStr, inet_ntoa(to.sin_addr), (int)ICMP_SIZE);
   ...
   return (nsend == nreceived)?0 : -1; //相等表示0% loss
}
